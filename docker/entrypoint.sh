...
if [ -z ${ALEO_PRIVKEY+a} ]; then 
	/aleo/bin/snarkos start --nodisplay --node ${SNARKOS_PORT} --rest ${RPC_PORT} --verbosity ${LOGLEVEL} 
else
	/aleo/bin/snarkos start --nodisplay --node ${SNARKOS_PORT} --rest ${RPC_PORT} --verbosity ${LOGLEVEL} --prover ${ALEO_PRIVKEY}
fi
